...
import type { Autocomplete } from "framework7/types";

export const AutocompletePage = () => {
  const fruits =
    "Apple Apricot Avocado Banana Melon Orange Peach Pear Pineapple".split(" ");

  const autocompleteDropdownSimple = useRef<Autocomplete.Autocomplete | null>(
    null
  );
  const autocompleteDropdownAll = useRef<Autocomplete.Autocomplete | null>(
    null
  );
  const autocompleteDropdownPlaceholder =
    useRef<Autocomplete.Autocomplete | null>(null);
  const autocompleteDropdownTypeahead =
    useRef<Autocomplete.Autocomplete | null>(null);
  const autocompleteDropdownAjax = useRef<Autocomplete.Autocomplete | null>(
    null
  );
  const autocompleteDropdownAjaxTypeahead =
    useRef<Autocomplete.Autocomplete | null>(null);
  const autocompleteStandaloneSimple = useRef<Autocomplete.Autocomplete | null>(
    null
  );
  const autocompleteStandalonePopup = useRef<Autocomplete.Autocomplete | null>(
    null
  );
  const autocompleteStandaloneMultiple =
    useRef<Autocomplete.Autocomplete | null>(null);
  const autocompleteStandaloneAjax = useRef<Autocomplete.Autocomplete | null>(
    null
  );
  ...
  return (
    ...
    >
      ...
      <Block>
        <p>
          Dropdown autocomplete is good to use as a quick and simple solution to
          provide more options in addition to free-type value.
        </p>
      </Block>
      ...
      <Block>
        <p>
          Standalone autocomplete provides better mobile UX by opening it in a
          new page or popup. Good to use when you need to get strict values
          without allowing free-type values.
        </p>
      </Block>
